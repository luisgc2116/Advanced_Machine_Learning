...
def VGG16_AvgPool(shape):
  # Recreate VGG16 maxpool to avg pool. This is bc max throws away information
  # we want to account for features across the entire image
  vgg = VGG16(input_shape=shape, weights='imagenet', include_top=False)

  new_model = Sequential()
  for layer in vgg.layers:
    # Replace max to avg pooling
...
def unpreprocess(img):
  # VGG expects different format than RGB for matplotlib
  # Basically does reverse of Keras preprocess input
  img[..., 0] += 103.939
  img[..., 1] += 116.779
  img[..., 2] += 126.68
  img = img[..., ::-1]
...
def scale_img(x):
  # For matplotlib scaling, since it only accepts values [0,1]
  x = x - x.min()
  x = x / x.max()
  return x
...
if __name__ == '__main__':
  ...
  t0 = datetime.now()
  losses = []
  x = np.random.randn(np.prod(batch_shape)) #init guess into minimizer
  for i in range(10):
    x, l, _ = fmin_l_bfgs_b(
